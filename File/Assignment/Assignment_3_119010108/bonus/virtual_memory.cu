...
__device__ void init_invert_page_table(VirtualMemory *vm) {
  ...
  for (int i = 0; i < vm->PAGE_ENTRIES; i++) {
	...
	vm->invert_page_table[i + vm->PAGE_ENTRIES*2] = -1; // initialize with -1
  }
}
...
__device__ bool ISPageFault(VirtualMemory *vm, u32 page_num) {
	...
	for (int i = 0; i < vm->PAGE_ENTRIES; i++) {
		if (vm->invert_page_table[i] == page_num && vm->invert_page_table[i + 2*vm->PAGE_ENTRIES] == threadIdx.x) {
			return false;
		}
	}
...
__device__ void UpdateTable(VirtualMemory *vm, u32 frame_num) {
	...
	vm->invert_page_table[2 * vm->PAGE_ENTRIES + frame_num] =threadIdx.x;
}
