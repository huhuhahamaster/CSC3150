...
__device__ __managed__ int current_thread = 0;
...
__global__ void mykernel(int input_size) {
  ...
  for (int current = 0; current < Thread_Num; current++){
    if (current == rank){
      user_program(&vm, input, results, input_size);
      current_thread = rank + 1;
    }
    __syncthreads();
  }
}
...
int main() {
  ...
  mykernel<<<1, Thread_Num, INVERT_PAGE_TABLE_SIZE>>>(input_size);      /* launch 4 threads */
